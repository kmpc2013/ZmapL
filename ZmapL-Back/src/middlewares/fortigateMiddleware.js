const validateFieldFortigate = (req, res, next) => {
    const { body } = req;

    // VDOM
    if (body.VDOM.trim().length > 200) {
        return res.status(400).json({ message: 'The field "VDOM" is to long' });
    }
